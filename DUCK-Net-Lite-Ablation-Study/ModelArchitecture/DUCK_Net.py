...
def create_model(img_height, img_width, input_chanels, out_classes, starting_filters):
    ...
    t0 = conv_block_2D(input_layer, starting_filters, 'conv', repeat=1)

    l1i = Conv2D(starting_filters * 2, 2, strides=2, padding='same')(t0)
    s1 = add([l1i, p1])
    t1 = conv_block_2D(s1, starting_filters * 2, 'conv', repeat=1)

    l2i = Conv2D(starting_filters * 4, 2, strides=2, padding='same')(t1)
    s2 = add([l2i, p2])
    t2 = conv_block_2D(s2, starting_filters * 4, 'conv', repeat=1)

    l3i = Conv2D(starting_filters * 8, 2, strides=2, padding='same')(t2)
    s3 = add([l3i, p3])
    t3 = conv_block_2D(s3, starting_filters * 8, 'conv', repeat=1)

    l4i = Conv2D(starting_filters * 16, 2, strides=2, padding='same')(t3)
    s4 = add([l4i, p4])
    t4 = conv_block_2D(s4, starting_filters * 16, 'conv', repeat=1)
   ...
   # --- BOTTLE‑NECK GIỮ NGUYÊN (t4: F*16 @14×14) -----------------------------
    ...
    q3  = conv_block_2D(c3, starting_filters * 8, 'conv', repeat=1)
    ...
    q2  = conv_block_2D(c2, starting_filters * 4, 'conv', repeat=1)
